...
const _: () = {
    ...
    #[allow(non_camel_case_types)]
    pub struct __ink_FetchRandomInstance {
        __ink_private: __ink_Private,
    }
    impl __ink_FetchRandomInstance {
        #[doc = " Note: this gives the operation a corresponding `func_id` (1101 in this case),"]
        #[doc = " and the chain-side chain extension will get the `func_id` to do further operations."]
        #[inline]
        pub fn fetch_random(
            self,
        ) -> ::core::result::Result<Result<u8, RandomReadErr>, RandomReadErr> {
            ::ink_env::chain_extension::ChainExtensionMethod::build(1101u32)
                .input::<()>()
                .output::<Result<u8, RandomReadErr>>()
                .handle_error_code::<RandomReadErr>()
                .call(&())
        }
        #[inline]
        pub fn test(
    ...
    impl ::ink_lang::ChainExtensionInstance for FetchRandom {
        type Instance = __ink_FetchRandomInstance;
...
impl PalletAsset {
    fn test() -> core::result::Result<Result<u8, RandomReadErr>, RandomReadErr> {
        FetchRandom::instantiate().test()
    }
}
...
pub mod my_psp22 {
    ...
    impl MyPSP22 {
		...
		#[ink(message)]
        pub fn update(&self, t: i32) -> Result<u8, RandomReadErr> {
            // Get the on-chain random seed
            if t > 0 {
                PalletAsset::test()?
                // self.env().extension().test()?
            } else {
                self.env().extension().fetch_random()?
            }
        }
