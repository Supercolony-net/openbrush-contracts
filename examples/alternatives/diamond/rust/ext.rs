...
mod sys {
    ...
    extern "C" {
        ...
        pub fn seal_deposit_event(topics_ptr: Ptr32<[u8]>, topics_len: u32, data_ptr: Ptr32<[u8]>, data_len: u32);
        ...
        pub fn seal_weight_to_fee(gas: u64, output_ptr: Ptr32Mut<[u8]>, output_len_ptr: Ptr32Mut<u32>);
        ...
        pub fn seal_hash_keccak_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
        pub fn seal_hash_blake2_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
        pub fn seal_hash_blake2_128(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
        pub fn seal_hash_sha2_256(input_ptr: Ptr32<[u8]>, input_len: u32, output_ptr: Ptr32Mut<[u8]>);
        ...
        pub fn seal_own_code_hash(output_ptr: Ptr32Mut<[u8]>, output_len_ptr: Ptr32Mut<u32>);
        ...
        pub fn seal_ecdsa_to_eth_address(public_key_ptr: Ptr32<[u8]>, output_ptr: Ptr32Mut<[u8]>) -> ReturnCode;
    }
    ...
    extern "C" {
        ...
        pub fn seal_set_storage(key_ptr: Ptr32<[u8]>, value_ptr: Ptr32<[u8]>, value_len: u32) -> ReturnCode;
    }
}
...
pub fn call(flags: u32, callee: &[u8], gas_limit: u64, value: &[u8], input: &[u8], output: &mut &mut [u8]) -> Result {
...
pub fn input(output: &mut &mut [u8]) {
    let mut output_len = output.len() as u32;
    {
        unsafe { sys::seal_input(Ptr32Mut::from_slice(output), Ptr32Mut::from_ref(&mut output_len)) };
    }
    extract_from_slice(output, output_len as usize);
}

pub fn return_value(flags: u32, return_value: &[u8]) -> ! {
    unsafe { sys::seal_return(flags, Ptr32::from_slice(return_value), return_value.len() as u32) }
}
...
pub fn weight_to_fee(gas: u64, output: &mut &mut [u8]) {
    let mut output_len = output.len() as u32;
    {
        unsafe { sys::seal_weight_to_fee(gas, Ptr32Mut::from_slice(output), Ptr32Mut::from_ref(&mut output_len)) };
    }
    extract_from_slice(output, output_len as usize);
}
...
pub fn debug_message(message: &str) {
    ...
    if unsafe { DEBUG_ENABLED || FIRST_RUN } {
        ...
        let ret_code = unsafe { sys::seal_debug_message(Ptr32::from_slice(bytes), bytes.len() as u32) };
...
pub fn ecdsa_to_eth_address(pubkey: &[u8; 33], output: &mut [u8; 20]) -> Result {
    let ret_code = unsafe { sys::seal_ecdsa_to_eth_address(Ptr32::from_slice(pubkey), Ptr32Mut::from_slice(output)) };
    ret_code.into()
}
...
pub fn own_code_hash(output: &mut [u8]) {
    let mut output_len = output.len() as u32;
    unsafe { sys::seal_own_code_hash(Ptr32Mut::from_slice(output), Ptr32Mut::from_ref(&mut output_len)) }
}
