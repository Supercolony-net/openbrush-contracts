...
Customize it by adding logic for adding new token types which can be minted. We can register a new token id with the `add_type` function, and this id will be added to `registered_ids` and when new tokens are about to be minted, we will first check if the type of minted token is registered in the smart contract.
