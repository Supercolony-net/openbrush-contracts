...
pub(crate) fn impl_external_trait(mut impl_item: syn::ItemImpl,
    ...
    let ink_methods_iter = ink_methods.iter().map(|(_, value)| value);
    let wrapper_trait_ident = format_ident!("{}{}", trait_ident, WRAPPER_TRAIT_SUFFIX);
    ...
    let external_ink_methods_iter = ink_methods.iter_mut().map(|(_, value)| {
        value.sig.ident = format_ident!("{}{}", value.sig.ident, EXTERNAL_METHOD_SUFFIX);
        value
    });
    let external_trait_ident = format_ident!("{}{}", trait_ident, EXTERNAL_TRAIT_SUFFIX);
