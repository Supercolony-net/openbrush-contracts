...
describe('MY_PAYMENT_SPLITTER', () => {
  async function setup() {
    ...
    const { address: contractAddress } = await contractFactory.new([
      [kayne.address, KAYNE_SHARE],
      [ian.address, IAN_SHARE]
    ])
  ...
  it('PAYMENT SPLITTER - init values', async () => {
    ...
    const kayneShares = (await contract.query.shares(kayne.address)).value.unwrapRecursively().toNumber()
    await expect(kayneShares).to.be.eq(40)
    const ianShares = (await contract.query.shares(ian.address)).value.unwrapRecursively().toNumber()
    await expect(ianShares).to.be.eq(60)
    const totalShares = (await contract.query.totalShares()).value.unwrapRecursively().toNumber()
  ...
  it('PAYMENT SPLITTER - release a native token', async () => {
    ...
    const kayneReleased = (await contract.query.released(kayne.address)).value.unwrapRecursively().toNumber()
    const ianReleased = (await contract.query.released(ian.address)).value.unwrapRecursively().toNumber()
