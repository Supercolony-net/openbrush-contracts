...
describe('MY_OWNABLE', () => {
  ...
  it('PSP 1155 - safe batch transfer works', async () => {
    ...
    await expect(
      query.balanceOfBatch([contract.address, sender.address, contract.address, sender.address], [bnArg(0), bnArg(0), bnArg(1), bnArg(1)])
  ...
  it('PSP 1155 - receiver can reject the transfer', async () => {
    ...
    await expect(query.balanceOfBatch([contract.address, sender.address], [bnArg(0), bnArg(0)])).to.have.output([0, 1])
  })
  ...
  it('PSP 1155 - approved for all works', async () => {
    ...
    await expect(query.balanceOfBatch([sender.address, sender.address], [bnArg(0), bnArg(1)])).to.have.output([1, 1])
    ...
    await expect(fromSigner(contract, alice.address).tx.safeBatchTransferFrom(sender.address, bob.address, [bnArg(0), bnArg(1)], [1, 1], 'data')).to
      .eventually.be.fulfilled

    // Assert - Bob owns the two tokens+
    await expect(query.balanceOfBatch([bob.address, bob.address], [bnArg(0), bnArg(1)])).to.have.output([1, 1])

    // Assert - Bob is now owner of the tokens: check that sender cannot transfer tokens on his behalf
    await expect(fromSigner(contract, alice.address).tx.safeBatchTransferFrom(bob.address, alice.address, [bnArg(0), bnArg(1)], [1, 1], 'data')).to
