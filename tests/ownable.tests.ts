...
describe('MY_OWNABLE', () => {
  ...
  it('PSP 1155 - burn works', async () => {
    ...
    await expect(contract.tx.burn(bnArg(0), 1)).to.eventually.be.fulfilled
  ...
  it('PSP 1155 - transfer works', async () => {
    ...
    await expect(contract.tx.mint(bnArg(0), 100)).to.eventually.be.fulfilled
  ...
  it('PSP 1155 - safe batch transfer works', async () => {
    ...
    await expect(tx.mint(bnArg(0), 100)).to.eventually.be.fulfilled
    await expect(tx.mint(bnArg(1), 100)).to.eventually.be.fulfilled

    // Act - Sender transfer 20 of token 0 and 70 of token 1
    await expect(contract.query.getCallCounter()).to.have.output(0)
    await expect(tx.safeBatchTransferFrom(sender.address, contract.address, [[bnArg(0), 20], [bnArg(1), 70]], 'data')).to.eventually.be.fulfilled
    await expect(contract.query.getCallCounter()).to.have.output(1)

    // Assert - Receiver own 20 of token 0 and own 70 of token 1
    await expect(
      query.balanceOfBatch([[contract.address, bnArg(0)], [sender.address, bnArg(0)], [contract.address, bnArg(1)], [sender.address, bnArg(1)]])
    ).to.have.output([20, 80, 70, 30])
  })
  ...
  it('PSP 1155 - approved for all works', async () => {
    ...
    await expect(contract.tx.mint(bnArg(0), 1)).to.eventually.be.fulfilled
    await expect(contract.tx.mint(bnArg(1), 1)).to.eventually.be.fulfilled
    await expect(query.balanceOfBatch([[sender.address, bnArg(0)], [sender.address, bnArg(1)]])).to.have.output([1, 1])
    ...
    await expect(fromSigner(contract, alice.address).tx.safeBatchTransferFrom(sender.address, bob.address, [[bnArg(0), 1], [bnArg(1), 1]], 'data')).to
      .eventually.be.fulfilled

    // Assert - Bob owns the two tokens+
    await expect(query.balanceOfBatch([[bob.address, bnArg(0)], [bob.address, bnArg(1)]])).to.have.output([1, 1])

    // Assert - Bob is now owner of the tokens: check that sender cannot transfer tokens on his behalf
    await expect(fromSigner(contract, alice.address).tx.safeBatchTransferFrom(bob.address, alice.address, [[bnArg(0), 1], [bnArg(1), 1]], 'data')).to
