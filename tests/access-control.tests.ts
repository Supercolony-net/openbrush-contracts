...
describe('MY_ACCESS_CONTROL', () => {
  ...
  it('PSP 721 - approved transfer works', async () => {
    ...
    await expect(fromSigner(contract, alice.address).tx.transferFrom(sender.address, bob.address, bnArg(0))).to.eventually.be.fulfilled
  ...
  it('PSP 721 - approved for all works', async () => {
    ...
    await expect(fromSigner(contract, alice.address).tx.transferFrom(sender.address, bob.address, bnArg(0))).to.eventually.be.fulfilled
    await expect(fromSigner(contract, alice.address).tx.transferFrom(sender.address, bob.address, bnArg(1))).to.eventually.be.fulfilled
    ...
    await expect(fromSigner(contract, sender.address).tx.transferFrom(bob.address, alice.address, bnArg(0))).to.eventually.be.rejected
  })
