...
describe('MY_ACCESS_CONTROL', () => {
  ...
  it('ACCESS CONTROL - only minter role is allowed to mint', async () => {
    ...
    await expect(fromSigner(contract, alice.address).tx.mint(1)).to.eventually.be.rejected
    ...
    await expect(fromSigner(contract, alice.address).tx.mint(0)).to.eventually.be.fulfilled
  ...
  it('ACCESS CONTROL - should reject burn if no minter role', async () => {
    ...
    await expect(fromSigner(contract, alice.address).tx.burn(alice.address, 0, 1)).to.eventually.be.rejected
  })
})
