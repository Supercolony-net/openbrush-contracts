...
describe('MY_PSP34', () => {
    ...
    it('Transfer works', async () => {
        ...
        await contract.tx.transferFrom(sender.address, alice.address, bnArg(0, 1), [])
    ...
    it('Transfer from works', async () => {
        ...
        await fromSigner(contract, alice.address).tx.transferFrom(sender.address, alice.address, bnArg(0, 1), [])
    ...
    it('PSP 34 - safe transfer works', async () => {
        ...
        await expect(tx.mintToken()).to.be.fulfilled
        await expect(query.ownerOf(bnArg(0, 1))).to.have.output(sender.address)
        ...
        await expect(query.ownerOf(bnArg(0, 1))).to.have.output(contract.address.toString())
    })
    ...
    it('PSP 34 - safe transfer works to contract that implements PSP34Receiver', async () => {
        ...
        await expect(tx.transferFrom(sender.address, contract.address, bnArg(0, 1), 'data')).to.eventually.be.fulfilled
        await expect(query.ownerOf(bnArg(0, 1))).to.have.output(contract.address)
    })
    ...
    it('Can not transfer non-existing token', async () => {
        ...
        await expect(contract.tx.transferFrom(sender.address, receiver.address, bnArg(0, 1))).to.eventually.be.rejected
    ...
    it('Can not transfer without allowance', async () => {
        ...
        await expect(fromSigner(contract, alice.address).tx.transferFrom(sender.address, alice.address, bnArg(0, 1), []))
