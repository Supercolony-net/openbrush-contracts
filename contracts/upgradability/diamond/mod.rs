...
impl<T: DiamondStorage + OwnableStorage + Flush> Diamond for T {
    ...
    default fn facets(&self) -> Vec<(Hash, Vec<Selector>)> {
        ...
        for i in 0..DiamondStorage::get(self).code_hashes {
            let hash = DiamondStorage::get(self).id_to_hash.get(&i).unwrap();
    ...
    default fn facet_code_hashes(&self) -> Vec<Hash> {
        let mut out_vec = Vec::new();
        for i in 0..DiamondStorage::get(self).code_hashes {
            out_vec.push(DiamondStorage::get(self).id_to_hash.get(&i).unwrap())
        }
        out_vec
    }
...
impl<T: DiamondStorage + OwnableStorage + Flush> DiamondInternal for T {
    fn _diamond_cut(&mut self, diamond_cut: Vec<FacetCut>, init: Option<InitCall>) -> Result<(), DiamondError> {
    ...
    fn _add_function(&mut self, code_hash: Hash, selector: Selector) -> Result<(), DiamondError> {
        if DiamondStorage::get(self).selector_to_hash.get(selector).is_some() {
            return Err(DiamondError::FunctionAlreadyExists)
        }

        let mut vec = DiamondStorage::get(self)
            .hash_to_selectors
            .get(&code_hash)
            .unwrap_or_else(|| {
                let hash_id = DiamondStorage::get(self).code_hashes;
                DiamondStorage::get_mut(self).hash_to_id.insert(&code_hash, &hash_id);
                DiamondStorage::get_mut(self).id_to_hash.insert(&hash_id, &code_hash);
                DiamondStorage::get_mut(self).code_hashes += 1;
                Vec::<Selector>::new()
            });
        ...
        DiamondStorage::get_mut(self).hash_to_selectors.insert(&code_hash, &vec);
        DiamondStorage::get_mut(self)
            .selector_position
    ...
    fn _remove_function(&mut self, selector: Selector) -> Result<(), DiamondError> {
        let code_hash = DiamondStorage::get(self)
            .selector_to_hash
            .get(selector)
            .ok_or(DiamondError::FunctionDoesNotExist)?;

        if code_hash == DiamondStorage::get(self).self_hash {
            return Err(DiamondError::ImmutableFunction)
        }

        let selector_pos = DiamondStorage::get(self).selector_position.get(&selector).unwrap();
        let mut selector_vec = DiamondStorage::get(self).hash_to_selectors.get(&code_hash).unwrap();
        ...
        if last_selector != selector {
            ...
            DiamondStorage::get_mut(self)
                .selector_position
                .insert(&last_selector, &selector_pos);
        }

        // if the vector of selectors is empty we can remove the hash
        if selector_vec.is_empty() {
            // we get id of our hash
            let hash_id = DiamondStorage::get(self).hash_to_id.get(&code_hash).unwrap();
            // we will decrease the count of hashes
            let last_id = DiamondStorage::get(self).code_hashes - 1;
            DiamondStorage::get_mut(self).code_hashes = last_id;
            // we are removing our hash so no need to track its id
            DiamondStorage::get_mut(self).hash_to_id.remove(&code_hash);
            // if removed hash was not the last added hash we need to change ids
            if hash_id != last_id {
                // current number of hashes is the id of hash
                let last_hash = DiamondStorage::get(self).id_to_hash.get(&last_id).unwrap();
                // change the id of last hash to id of currently removed hash
                DiamondStorage::get_mut(self).id_to_hash.insert(&hash_id, &last_hash);
            } else {
                // else we just remove the id
                DiamondStorage::get_mut(self).id_to_hash.remove(&hash_id);
            }
            ...
            DiamondStorage::get_mut(self)
                .hash_to_selectors
                .insert(&code_hash, &selector_vec);
        }

        DiamondStorage::get_mut(self).selector_to_hash.remove(&selector);
        DiamondStorage::get_mut(self).selector_position.remove(&selector);
